...
function Tela_SolicitarExames({ navigation }) {
  ...
  return (    
    <View style={styles.container}>
      ...
      {/* Título */}
      <View style={{flex: 0.08, width: '100%', alignItems: 'center', alignContent: 'center', backgroundColor: '#a2a2a2'}}>
        <Text style={{fontSize: 20, marginTop: 15}}>{'Informações do Paciente'}</Text>
      </View>

      {/* Campos */}
      <View style={{flex: 0.92, width: '100%', alignItems: 'center', backgroundColor: 'white'}}>
        <ScrollView style={{width: '100%'}}>

          {/* Idade */}
          <View>
            <Text style={{marginLeft: 12, marginTop: 18}}>{'Idade'}</Text>
            <TextInput style={styles.input} placeholder="18" inputMode="numeric"/>
          </View>
          
          {/* Gênero */}
          <View>
            <Text style={{marginLeft: 12}}>{'Gênero'}</Text>
            <Picker style={{marginLeft: 12, marginTop: 10, width: '93%', borderColor: 'black'}} selectedValue={genero} onValueChange={handleValueChange} mode='dropdown'>
